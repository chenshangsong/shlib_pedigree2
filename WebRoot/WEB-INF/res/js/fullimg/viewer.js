(function($){
  ...
  $.GjCatchEndpoint = function(options) {
  ...
  $.GjCatchEndpoint.prototype = {
    ...
    deleteAnnotation: function(annotationID, successCallback, errorCallback) {
      var _this = this;
      jQuery.ajax({
       url: this.prefix+"/destroy" + this.params,
       type: 'GET',
       dataType: 'json',
       data:{
    	   id : annotationID
    ...
    update: function(oaAnnotation, successCallback, errorCallback) {
      ...
      annotations.forEach(function(annotation) {
        ...
        jQuery.ajax({
          url: _this.prefix+"/update" + _this.params,
          type: 'POST',
          dataType: 'json',
          headers: {
    ...
    create: function(oaAnnotation, successCallback, errorCallback) {
        console.log(JSON.stringify(oaAnnotation));
      var _this = this,
    ...
    createCatchAnnotation: function(catchAnnotation, successCallback, errorCallback) {
      var _this = this;
      console.log(JSON.stringify(catchAnnotation));
      jQuery.ajax({
...
$(function() {
	...
	var userId = $("#user").val();
	var userName = "";
	var moduleS = (userId == '' ? 'LocalStorageEndpoint' : 'GjCatchEndpoint');
	// fullimg地址
	var fullimgPath = "http://fullimg.library.sh.cn/manifests/";
	// 内外网标识
	var isInnerIP = $("#isInnerIP").val();
	if(isInnerIP=="1"){
		//fullimgPath = "http://10.1.30.112/manifests/"
		fullimgPath = "http://fullimgin.library.sh.cn/manifests/";
			
	}
	/*var moduleS = 'GjCatchEndpoint';*/
    myMiradorInstance = Mirador({
      id: "viewer",
      layout: "1x1",
      buildPath: ctx+"/res/mirador/",
      data: [
        { manifestUri: fullimgPath + org + "/"+ manifest +".json", location: "Shanghai Library"},
      ],
		 mainMenuSettings: {
			show: false
		  },
      windowObjects: [{
			loadedManifest: fullimgPath + org + "/"+ manifest +".json",
			viewType: "ImageView",
			//displayLayout: false,
			//bottomPanel: false,
