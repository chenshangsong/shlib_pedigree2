$(function(){
    ...
    var loadPersonList = function(){
        ...
        $.post(ctx + "/service/person/list", $("#listForm").serialize(), function(result){
            $("#pager").html(pager.paint(result.pager));
            $(".data_list").hide().html(juicer(getTemplate("persons"), result)).fadeIn("slow");
            hideLoading();
        });
    };
    ...
    $(".data_list").on("click", "a[view]", function(){
        ...
        var view = $(this).attr("view");
         window.open(ctx+"/service/work/persons?uri="+uri);
        return false;
    });
  /*弹框形式  $(".data_list").on("click", "a[view]", function(){
        showLoading();
        var uri = $(this).attr("href");
        var view = $(this).attr("view");
