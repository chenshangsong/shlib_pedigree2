$(function() {
	...
	var raster = new ol.layer.Tile({
		 //source: new ol.source.OSM()chenssmap
		source: new ol.source.BingMaps({
            key: 'AmqeuOCB8LKuzGAuQKuDxZdidXcm-OJZYnWwLY4uTprkxAsWRDDfrP_XTqCa76Vc',
            imagerySet: 'Road'
        })
	});
  ...
  //天地图end
	var map = new ol.Map({
		...
		target : $('#map')[0],
		view : new ol.View({
			center : ol.proj.transform([ 103.23, 34.33 ], 'EPSG:4326',
					'EPSG:3857'),
			zoom : 4
		})
	,
		...
		if (value !== 'None') {
			...
			draw.on("drawend", function(evt) {
				...
				for (var i = 0; i < k.length; i += 2) {
					points.push(ol.proj.transform([ k[i], k[i + 1] ],
							'EPSG:3857', 'EPSG:4326'));
				}
	...
	var loadPlacePoints = function(points) {
		...
		$.get(ctx + "/service/place/listInArea", {
			points : pointStr,
			familyName : familyName
		}, function(data) {
			drawPlacePoints(data)
		});
	}
	...
	map.on('click', function(evt) {
		...
		if (feature) {
			...
			$.post(ctx + "/service/work/list", {
				place_uri : feature.get("uri"),
				familyName : familyName,
				pageSize : 10
			}, function(data) {
				...
				$.each(works, function(i) {
					html += '<a href="' + works[i].work + '" tag="work">'
							+ works[i].title + "</a><br>";
				})
				if (html == "")
					html = "暂无文献数据"
				if (data.pager.pageCount > 1) {
					html += '<a href="javascript:void(0);" place="'
							+ feature.get("name") + '"familyName="' + familyName
							+ '" tag="more"><i>更多...</i></a>';
				}
				$(element).popover({
					placement : 'right',
	...
	$("#btn_polygon").click(
			function() {
				var self = $(this);
				self.hasClass("active") ? (function() {
					removeDraw();
					self.find("i").removeClass("glyphicon-check").addClass(
							"glyphicon-screenshot")
				})() : (function() {
					addDraw();
					self.find("i").removeClass("glyphicon-screenshot")
	...
	$("#map").on(
			...
			function() {
				location.href = ctx + "/service/work/list#place="
						+ $(this).attr("place") + "&familyName="
						+ $(this).attr("familyName");
			})
	$(document.body).on("click", "a[tag=work]", function() {
		window.open(ctx + "/service/work/list#uri=" + $(this).attr("href"));
		return false;
		/*
		 * var uri = $(this).attr("href"); showLoading(); $.get(ctx +
		 * "/service/work/get", {uri : uri}, function(data){ hideLoading();
		 * if(!data){ Dialog.error("未查询到文献信息");return;} var html =
		 * juicer(getTemplate("work"), data); $("#workModal
		 * .modal-title").text("家谱文献");
		 * $("#workModal").modal("show").find(".modal-body").html(html); });
		 * return false;
		 */
	}).on("click", "a[tag=place]", function() {
		// location.href = ctx + "/service/place/list#place=" +
		// $(this).attr("href");
