$(function(){
    ...
    $("#workForm").submit(function(){
        ...
        $.post(ctx + '/service/work/countItemByWork', $(this).serialize(), function(result){
            $("#itemCount").html(result.count);
        });
        return false;
    });
    //加载删除方法
function loaddeleteFun(){
	$(".btnDelete").click(function(){
		deleteWorkUri=$(this).attr('rel');
		//消息提示
	   showConfirmMsg(String.format(IMSG0003,"删除该书目"),saveFunc);
     });
   //保存函数
   function saveFunc(){
		//保存work
		requestJson(ctx+"/workManager/doDeleteVt?workUri="+deleteWorkUri, null,
				successFun);
      };
   ...
   function getpxtData(){
   		var thisUri = $("#hidUri").val();
   var thisPersonUri = '';
		$("#btnPxList").hide(); 
		//胡适
		if(thisUri=='http://data.library.sh.cn/jp/resource/work/jklhb5c3ga1rvxe3'){
			thisPersonUri='http://data.library.sh.cn/entity/person/wuzpqbpa6d4j4qd5';
			$("#btnPxList").show();
			$("#btnPxList").attr('href','../work/dendrogram?uri='+thisPersonUri);
		}
		//盛宣怀
		else if (thisUri=='http://data.library.sh.cn/jp/resource/work/zxww5ignvetnkd8d'){
			thisPersonUri='http://data.library.sh.cn/entity/person/4lnpsgh342b2n1ru';
			$("#btnPxList").show();
			$("#btnPxList").attr('href','../work/dendrogram?uri='+thisPersonUri);
			
		}
		//李鸿章
		else if (thisUri=='http://data.library.sh.cn/jp/resource/work/kkgv5g289866rbm4'){
			//thisPersonUri='http://data.library.sh.cn/jp/entity/person/1693lhvl7lk3j1fn';
			thisPersonUri='http://data.library.sh.cn/entity/person/7la7uay6h4ehfawq';
			$("#btnPxList").show();
			$("#btnPxList").attr('href','../work/dendrogram?uri='+thisPersonUri);
    ...
    var showWork = function(uri){
        ...
        $.get(ctx + "/service/work/get", {uri : uri}, function(data){
            ...
            //隐藏高级查询按钮
                $(".Search").html('');
               $("#top span").attr("text", $("#top span").text()).show().html('<img src="' + ctx + '/res/images/work.jpg" style="width: 50px;float:left;margin-top:-10px;"/>' + data.dtitle);
        	
            // 新增反馈信息chenss
    	/*if(ctxRoleId!=$.roleGroup.normal){
            $("#top span").append('<a class="adminDiv" href="../../workManager/dataContentlist?id=' + uri + '&dataTypeId=10"  target="_blank" ><i class="icon-edit"></i></a>');
    	}*/
            $("#hidUri").val(uri);
             //谱系图数据
             getpxtData();
            $.each(data.places, function(i){
                $("#top span").append('&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<small><a href="' + data.places[i].place + '" tag="place"><i class="glyphicon glyphicon-map-marker"></i>' + data.places[i].label + '</a></small>');
            });
           if(uri.indexOf("11267")!=-1 || uri.indexOf("11268")!=-1){
                $("#btn_mig").show();
                $("#btn_tree").show();
            }
            $("#workForm").hide();
...
// return '<a href="javascript:void(0);" data-container="body"
// data-toggle="popover" data-trigger="focus" data-placement="right"
// data-title="' + data.name + '" data-content="' +(data.start?("【" + data.start
// + "】\r\n"):"") + (data.descs||'无摘要') + '">' + data.name + '</a>'
        ...
        // $("#tree_container .tree").css("width", $("#tree_container
		// .tree>ul>li").width())
