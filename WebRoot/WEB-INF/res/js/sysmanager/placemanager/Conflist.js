$(function(){
    ...
    $.get(ctx + "/placeManager/listPlaces", function(data){
        ...
        $.each(provs, function(i){
            var prov = provs[i];
            $("<h3>").html("<a onClick='aClick(this)' auri='" + prov.uri + "' aname='" + prov.prov  + "' href='javacript:void(0)'>" + prov.prov + "</a>").appendTo(container);
            ...
            $.each(prov.cities, function(j){
                ...
                if(city.city == prov.prov){
                    ...
                    $("<li>").appendTo(prov1).html("<a onClick='aClick(this)' auri='" + city.uri + "' aname='" + city.city + "' href='javacript:void(0)'>" + city.city + "</a>");
                ...
                $.each(city.counties, function(k){
                    ...
                    $("<li>").appendTo(city1).html("<a onClick='aClick(this)' auri='" + county.uri + "' aname='" + county.county + "' href='javacript:void(0)'>" + county.county + "</a>");
    ...
    $.get(ctx + "/placeManager/listOrigin", function(data){
        ...
        $.each(provs, function(i){
            ...
            $.each(provs[i].value, function(j){
                html += "<li><a onClick='aClick(this)' auri='" + provs[i].value[j]['uri'] + "' aname='" + provs[i].value[j]['label'] + "' href='javacript:void(0)'>" + provs[i].value[j]['label'] + "</a></li>";
            });
            html += "</ul>";
        });
        $("#originDiv").html(html);
    ...
    var showView = function(obj, self, uri){
        showLoading();
        $.get(ctx + "/placeManager/view", {uri:uri}, function(data){
            placeData = data;
                    ...
                    for(var w=0;w<workPoints.length;w++) {
                        ...
                        $.each(points, function (j) {
                            ...
                            if (point.x == x && point.y == y) {
                                point.content += "<br><a href='" + work.work + "' tag='work' class='text-nowrap'>" + work.title + "</a>";
                                exists = true;
                                return false;
                            }
                        })
                        if (!exists&&x&&y) {
                            $.ajax({
                                url: ctx + "/placeManager/name",
                                async: false,
                                data: {longx: x, lat: y},
                                success: function (label) {
                                    points.push({x: x, y: y, name: label, content:"</b><a href='" + work.work + "' tag='work' class='text-nowrap'>" + work.title + "</a>"});
                                }
                            });
                        }
                    }
    ...
    $(document.body).on("click", "a[tag=place]", function(){
        location.href = ctx + "/placeManager/list#place=" + $(this).attr("href");
