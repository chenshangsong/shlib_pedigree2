...
<body>
	...
	<div class="container">
		<div class="panel panel-default">
			...
			<input type='hidden' id="hidhistoryMainId" value="" /> <input
				type='hidden' id="hidId" class="formSaveMaster" name="id"
				value="${data1.id}" /> <input type='hidden' id="hidGraph"
				class="formSaveMaster" name="graphUri" value="${data1.graphUri}" />
			<input type='hidden' id="hidPageUrl" class="formSaveMaster"
				name="pageUrl" value="${data1.pageUrl}" /> <input type='hidden'
				id="hidDataType" class="formSaveMaster" name="dataType"
				value="${data1.dataType}" />
							...
							<tr class="text-left">
								...
								<td width='40%' style="padding: 3px;"><c:if
										...
										<c:forEach var="thisO" items="${fn:split(x.oldValue,',')}">
											...
											<textarea <c:if test="${x.oldEnName!='description'}">rows='1'</c:if><c:if test="${x.oldEnName=='description'}">rows='6'</c:if> type="text"
									...
									</c:if> <c:if test="${x.oldEnName=='address'}">
										<c:set var="str" value="${x.oldValue}" />
										<c:set var="strLang" value="strnone" />
										<c:choose>
											<c:when test="${fn:contains(str, '@chs')}">
												<c:set var="strLang" value="strchs" />
												<span style="float: left;">简体：</span>
												<c:set var="str" value="${fn:substringBefore(str,'@')}" />
											</c:when>
											<c:when test="${fn:contains(str, '@cht')}">
												<c:set var="strLang" value="strcht" />
												<span style="float: left;">繁体：</span>
												<c:set var="str" value="${fn:substringBefore(str,'@')}" />
											</c:when>
											<c:when test="${fn:contains(str, '@en')}">
												<c:set var="strLang" value="stren" />
												<span style="float: left;">英文：</span>
												<c:set var="str" value="${fn:substringBefore(str,'@')}" />
											</c:when>
										...
										<textarea rows='3' type="text"
											...
											style=" width:40%; <c:if test="${x.isUri=='1' }">display:none; </c:if>"
											placeholder="在此输入新值" />${str}</textarea>
									</c:if> <span class="spanNewValue" style="float: left;">${x.oldValueCn}</span>
	...
	var isRel='0',pageUrl ='', dataType='' ;
			...
			function successFun(data) {
				if (data.result == true) {
					...
					if(isRel=='1'){
						...
						requestJson("${ctx}/dataManager/doRelease?id="+id, null,reload);
		...
		function buttonC(){
			//不是管理员，保存并发布按钮隐藏
			var roleID = ${ctxRoleId.getRoleId()};
			if(roleID!=$.roleGroup.admin){
