				...
				<c:forEach var="x" varStatus="status" items="${data}">
					<tr class="text-left">
						...
						<td width='10%'>${x.remarks}<input type='hidden'
							...
							<input type='hidden'
							class="formSave addFlg" name="addFlg" value="${x.addFlg}" />
						</td>
...
<script>
	var dataId, pageUrl, rowIndex,dataType;
	//发布
	$(".btn_historyRelease").click(function() {
	    ...
	    showConfirmMsg(String.format(IMSG0003,"发布"),releaseFunc);
	   	function releaseFunc(){
			...
			dataType = $this.parents("tr").find(".datatype").val();
			var addFlg = $this.parents("tr").find(".addFlg").val();
			dataId = $this.parents("tr").find(".dataUri").val();
			var id = $this.parents("tr").find(".id").val();
			pageUrl = $.pageUrlGroup.getName(dataType);
			//如果是新增，则调用新增发布
			if (addFlg=='1'){
				requestJson("${ctx}/dataManager/doNewAddRelease?id="+id, null, successReleaseFun);
			}
			else{
				requestJson("${ctx}/dataManager/doRelease?id="+id, null, successReleaseFun);
		...
		if (data.result == true) {
			...
			parent.layer.msg('发布成功 ', {
				...
				parent.location.href = '${ctx}/' + pageUrl
						+ 'dataContentlist?id=' + dataId+'&dataTypeId='+dataType;
	...
	$(".btn_historyDelete").click(function() {
		var $this = $(this);
		//消息提示
	    showConfirmMsg(String.format(IMSG0003,"删除"),deleteFunc);
	   	function deleteFunc(){
	   		var formSave = $this.parents("tr").find(".formSave");
			...
			requestJson("${ctx}/dataManager/doDelete", param, successDeleteFun);
	   	}
	});
	...
	$(".btn_historyDetailed").click(function() {
		var id = $(this).parents("tr").find(".id").val();
		var strContent = '${ctx}/dataHistory/confPropertylist?id=' + id;
		//弹出姓氏搜索框
		layer.open({
			type : 2,
			title : "属性列表",
