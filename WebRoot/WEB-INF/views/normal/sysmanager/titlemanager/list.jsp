...
<div class="container">
    <h2 id="top"><span>上海图书馆家谱谱名列表</span> </h2>
    <hr/>
    ...
    <form method="post" id="workForm" action="${ctx}/titleManager/list" class="form-inline">
        <input type="hidden" name="pageSize" value="60" />
        <div>
            <div class="form-group">
                <label>谱名：</label>
                <input type="text" class="form-control" name="title" id="title" placeholder="在此输入谱名">
            </div>
...
<script type="text/html" id="worksTpl">
    {@each works as work, i}
        ...
        <div class="col-xs-3">
            <a href="${ctx}/titleManager/dataContentlist?id=@{work.uri}">@{work.title}</a>
        </div>
        {@if i % 4 == 3 || i == works.length - 1}</div>{@/if}
    {@/each}
    {@if works.length == 0}<div class="row"><div class="col-xs-12">未查询到相关谱籍信息！</div></div>{@/if}
</script>
...
<script src="${ctx}/res/js/sysmanager/titlemanager/list.js"></script>
</body>
</html>
