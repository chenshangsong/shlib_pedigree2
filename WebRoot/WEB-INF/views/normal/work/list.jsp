...
{@if work.imageFrontPath.length != 0} <img src="@{work.imageFrontPath}" style="width: 100px;">{@/if}   
                    ...
                    <div class="tab-content">
                        {@each instances as instance, st}
                          <div role="tabpanel" class="tab-pane @{st == 0 ? ' fade in active':''}" id="tab_@{st}">
                             ...
                             {@if instance['description']} @@{instance['description']?instance['description']:''}
{@/if}                            
{@if instance['ab'] == '上图'}  {@if jjflag == '1'} (胶卷暂无法借阅){@/if} {@/if}
                          </div>
                        {@/each}
                    </div>
                    {@/if}
...
var jiathis_config={
	...
	pic:"https://jpv1.library.sh.cn/jp/res/logo.png",
	showClose:true,
	shortUrl:true,
	hiservicere:false
}
