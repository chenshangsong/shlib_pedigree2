...
public abstract class BaseDaoImpl implements BaseDao {
	...
	private void init() {
		...
		/*
		 * Constant _cons = new Constant(); for (String key :
		 * _cons.GRAPH_MAP.keySet()) { model = modelCache.get(key); if (model ==
		 * null || model.isClosed()) { model = getModel(key);
		 * modelCache.put(key, model); } graph = new VirtGraph(key, dataSource);
		 * 
		 * }
		 */
