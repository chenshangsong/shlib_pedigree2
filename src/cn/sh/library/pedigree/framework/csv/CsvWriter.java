...
public class CsvWriter {
	...
	/**
	 * Creates a {@link com.tes.sys.framework.handler.csv.CsvWriter CsvWriter} object using a file
	...
	/**
	 * Creates a {@link com.tes.sys.framework.handler.csv.CsvWriter CsvWriter} object using a Writer
	 * to write data to.
	 * 
	 * @param outputStream
	...
	/**
	 * Creates a {@link com.tes.sys.framework.handler.csv.CsvWriter CsvWriter} object using an
	 ...
	 *                destination stream.
	 */
	public void write(String content, boolean preserveSpaces, boolean formatSapces)
			throws IOException {
		checkClosed();
		...
		if (userSettings.UseTextQualifier && !textQualify
				&& content.length() > 0 && preserveSpaces && formatSapces) {
	...
	public void writeRecord(String[] values, boolean preserveSpaces)
		...
		if (values != null && values.length > 0) {
			for (int i = 0; i < values.length; i++) {
				write(values[i], preserveSpaces,true);
			}
	...
	public void writeRecord(String[] values, boolean preserveSpaces, boolean formatSpaces)
			throws IOException {
		if (values != null && values.length > 0) {
			for (int i = 0; i < values.length; i++) {
				write(values[i], preserveSpaces,formatSpaces);
			}
