...
public class FullLink4ESJP {
	...
	public static Map<String,String> GetFullTextImg(FullLink4ESJPSearchBean fullLink4ESJPSearchBean) {
		...
		if (fullLink4ESJPSearchBean.isFlag()) { // 是否为多个DOI, true为单个
			...
			_map.put("linkPDF", GetFullTextImg_PDF(fullLink4ESJPSearchBean.getAccessLevelFlg(),fullLink4ESJPSearchBean.getDoi().substring(0, 10),fullLink4ESJPSearchBean.getHasimg(),fullLink4ESJPSearchBean.getIpAddress()));
			...
			if (!"9".equals(fullLink4ESJPSearchBean.getAccessLevelFlg())
				...
				if (tempV.startsWith("STJP") && tempV.length() >= 10) {
					...
					_map.put("linkPDF", GetFullTextImg_PDF(fullLink4ESJPSearchBean.getAccessLevelFlg(),tempV.substring(0, 10),fullLink4ESJPSearchBean.getHasimg(),fullLink4ESJPSearchBean.getIpAddress()));
		...
		if (!"9".equals(accessLevel) && !StringUtilC.isEmpty(accessLevel)) {
			if (doi.startsWith("STJP") && doi.length() >= 10) {
				...
				if ("0".equals(accessLevel) && "true".equals(hasimg)) {
					// 外网放开 chenss 20181109
					link = MessageFormat.format("https://jpv1.library.sh.cn/jp/full-img/{0}/{1}", "jiapu", doi.substring(0, 10));
					link = GetOutFullLinkTemp(link);
	...
	public static String GetFulltext(String DOI, String ipAddress) {
		...
		ipAddress = StringUtilC.isEmpty(ipAddress)?
					StringUtilC.getString(HttpUtil.getSessionValue(RequestFilter.threadLocalRequest.get(), "userIp")):ipAddress;
...
//				result[0] = "https://dhapi.library.sh.cn/pdfview?innerflag=1&dbname="+db; //chenss 20230215 pdf浏览插件升级
				...
				if (cnt > 0) {
					...
					if (!isInnerIP(ipaddress)) {
					    result[1] = "<a href=\"javascript:alert(&quot;请到上海图书馆家谱阅览室浏览电子全文&quot;)\"><img img border='0' width='20px' height='20px' src='"+Constant.ProjectPath+"/res/images/pdf2.png' alt='请到上海图书馆家谱阅览室浏览电子全文'  ></a>";
	...
	private static boolean isInnerIP(String ip) {
		...
		if (ip.equals("127.0.0.1"))
			bInnerIP = true;
		System.out.println("IP："+ip+";ipaddress[0]： "+ipaddress[0]+";ipaddress[1]： "+ipaddress[1]+";是否内网："+bInnerIP);
		return bInnerIP;
	}
