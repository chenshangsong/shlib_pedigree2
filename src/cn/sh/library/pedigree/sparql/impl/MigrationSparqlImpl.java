...
/*     */   {
/*  23 */     String query = Namespace.getNsPrefixString() + "SELECT ?uri ?name ?begin ?start_label (CONCAT(?sl, ' ', ?st) AS ?start_geo) ?dest_label (CONCAT(?dl, ' ' , ?dt) AS ?dest_geo) " + "WHERE {" + "   ?uri a shl:Migration ; " + "        bf:eventAgent/foaf:name ?name ; " + "        shl:locality ?dest ; " + "        shl:originalLocality ?start ; " + "        shl:temporal/shl:beginYear ?begin. " + "   ?start geo:long ?sl ; " + "          geo:lat ?st ; " + "          bf:label ?start_label ." + "   ?dest geo:long ?dl ; " + "         geo:lat ?dt ; " + "         bf:label ?dest_label ." + "} ORDER BY ASC(?begin)";
/*     */ 
...
/*     */   {
/*  44 */     String query = Namespace.getNsPrefixString() + "SELECT DISTINCT ?label ?long ?lat " + "WHERE { " + "   ?uri a shl:Migration ; " + "        ?p ?s ; " + "        shl:temporal/shl:beginYear ?begin . " + "   ?s geo:long ?long ; " + "      geo:lat ?lat ; " + "      bf:label ?label . " + "} order by asc(?begin)";
/*     */ 
...
/*     */   {
/*  60 */     String query = Namespace.getNsPrefixString() + "SELECT DISTINCT ?start ?dest " + "WHERE { " + "   ?uri a shl:Migration ; " + "        shl:locality ?dest_uri ; " + "        shl:originalLocality ?start_uri ; " + "        shl:temporal/shl:beginYear ?begin. " + "   ?start_uri bf:label ?start . " + "   ?dest_uri bf:label ?dest . " + "} order by asc(?begin)";
/*     */ 
...
/*     */   {
/*  76 */     String query = Namespace.getNsPrefixString() + "SELECT ?uri " + "WHERE {" + "   ?uri a shl:Person . " + "FILTER NOT EXISTS {?uri rel:childOf ?parent . }" + "}";
/*     */ 
...
/*     */   {
/*  94 */     String query = Namespace.getNsPrefixString() + "SELECT ?uri " + "WHERE {" + "   ?uri a shl:Person ; " + "        rel:childOf <" + uri + "> . " + "OPTIONAL { " + "   ?uri shl:orderOfSeniority ?order . " + "   ?seq rdfs:comment ?order ; rdfs:label ?num . } " + "} ORDER BY ?num ";
/*     */ 
...
/*     */   {
/* 196 */     String query = Namespace.getNsPrefixString() + "SELECT ?uri " + "WHERE {" + "   ?uri foaf:name '" + name + "' . " + "}";
/*     */ 
