...
/*    */   {
/* 23 */     String query = Namespace.getNsPrefixString() + "SELECT DISTINCT ?uri ?name ?begin ?end " + "WHERE {" + "   ?uri a shl:Temporal ;  " + "        bf:label ?name . " + "OPTIONAL {?uri shl:beginYear ?begin . }" + "OPTIONAL {?uri shl:endYear ?end . }" + "FILTER NOT EXISTS{?uri shl:monarchName ?t .}" + "} ORDER BY DESC(?begin)";
/*    */ 
