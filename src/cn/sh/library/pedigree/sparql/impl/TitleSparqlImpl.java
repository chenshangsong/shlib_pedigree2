...
/*    */     }
/*    */ 
/* 33 */     String countQuery = this.nsPrefix + "SELECT (COUNT(DISTINCT ?uri) AS ?count) " + "WHERE {" + "   ?uri a bf:WorkTitle ;" + "        bf:label ?chs . " + filter + "}";
/*    */ 
/* 40 */     String query = this.nsPrefix + "SELECT DISTINCT ?uri (sql:BEST_LANGMATCH(?chs, 'cht;q=0.8, chs;q=0.7, ;q=0.6, en;q=0.5, *;q=0.1', 'cht')) as ?title " + "WHERE {" + "   ?uri a bf:WorkTitle ; " + "        bf:label ?chs . " + filter + "} " + "OFFSET " + start + " LIMIT " + size;
/*    */ 
