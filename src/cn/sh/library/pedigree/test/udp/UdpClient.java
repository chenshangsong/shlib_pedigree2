...
class ClientImpl implements Runnable{
	...
	public void run() {
		try {
			...
			// 发送随机的数据
			byte[] btSend = new byte[]{1,2,3};
			DatagramPacket packet = new DatagramPacket(buffer, buffer.length, InetAddress.getByName("localhost"), 39169);
			...
			if(null != bt && bt.length > 0){
				System.out.println(uuid + "：收到服务端返回：" + Arrays.toString(bt));
			}
			Thread.sleep(1 * 1000);
		} catch (Exception e) {
			e.printStackTrace();
